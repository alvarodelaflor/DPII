...
(function(window, undefined) {
	...
	jQuery.extend({
		...
		parseHTML : function(data, context, scripts) {
			...
			// Single tag
			if ((parsed = rsingleTag.exec(data))) { return [
				context.createElement(parsed[1])
			]; }

			parsed = jQuery.buildFragment([
				data
			], context, scripts ? null : []);
	...
	jQuery.event = {
		...
		remove : function(elem, types, handler, selector, mappedTypes) {
			...
			for (t = 0; t < types.length; t++) {
				...
				for (j = 0; j < eventType.length; j++) {
					...
					if ((mappedTypes || origType === handleObj.origType) && (!handler || handler.guid === handleObj.guid) && (!namespaces || namespaces.test(handleObj.namespace))
		...
		dispatch : function(event) {
			...
			var i, j, cur, ret, selMatch, matched, matches, handleObj, sel, related, handlers = ((jQuery._data(this, "events") || {})[event.type] || []), delegateCount = handlers.delegateCount, args = core_slice
					.call(arguments), run_all = !event.exclusive && !event.namespace, special = jQuery.event.special[event.type] || {}, handlerQueue = [];
					...
					if (cur.disabled !== true || event.type !== "click") {
						...
						for (i = 0; i < delegateCount; i++) {
							...
							if (selMatch[sel] === undefined) {
								selMatch[sel] = handleObj.needsContext ? jQuery(sel, this).index(cur) >= 0 : jQuery.find(sel, this, null, [
									cur
								]).length;
							}
							if (selMatch[sel]) {
	...
	if (!jQuery.support.changeBubbles) {
		...
		jQuery.event.special.change = {
			...
			handle : function(event) {
				...
				if (this !== elem || event.isSimulated || event.isTrigger || (elem.type !== "radio" && elem.type !== "checkbox")) { return event.handleObj.handler.apply(this,
						arguments); }
			},
	...
	/*
	 * ! Sizzle CSS Selector Engine Copyright 2012 jQuery Foundation and other contributors Released under the MIT license http://sizzlejs.com/
	 */
	(function(window, undefined) {
		...
		matchExpr = {
			...
			"CHILD" : new RegExp("^:(only|nth|first|last)-child(?:\\(" + whitespace + "*(even|odd|(([+-]|)(\\d*)n|)" + whitespace + "*(?:([+-]|)" + whitespace + "*(\\d+)|))"
					+ whitespace + "*\\)|)", "i"),
		...
		Expr = Sizzle.selectors = {
			...
			preFilter : {
				...
				"CHILD" : function(match) {
					/*
					 * matches from matchExpr["CHILD"] 1 type (only|nth|...) 2 argument (even|odd|\d*|\d*n([+-]\d+)?|...) 3 xn-component of xn+y argument ([+-]?\d*n|) 4 sign of
					 * xn-component 5 x of xn-component 6 sign of y-component 7 y of y-component
					 */
			...
			filter : {
				...
				"ATTR" : function(name, operator, check) {
					return function(elem, context) {
						...
						return operator === "=" ? result === check : operator === "!=" ? result !== check : operator === "^=" ? check && result.indexOf(check) === 0
								: operator === "*=" ? check && result.indexOf(check) > -1 : operator === "$=" ? check && result.substr(result.length - check.length) === check
				...
				"CHILD" : function(type, argument, first, last) {
					...
					if (type === "nth") { return function(elem) {
		...
		function addCombinator(matcher, combinator, base) {
			...
			function(elem, context, xml) {
				while ((elem = elem[dir])) {
					if (checkNonElements || elem.nodeType === 1) { return matcher(elem, context, xml); }
				}
			} :
			...
			function(elem, context, xml) {
				...
				if (!xml) {
					...
					while ((elem = elem[dir])) {
						if (checkNonElements || elem.nodeType === 1) {
							if (matcher(elem, context, xml)) { return elem; }
						}
					}
				}
			};
			...
			for (; i < len; i++) {
				if ((matcher = Expr.relative[tokens[i].type])) {
					...
					if (matcher[expando]) {
						...
						return setMatcher(i > 1 && elementMatcher(matchers), i > 1 && tokens.slice(0, i - 1).join("").replace(rtrim, "$1"), matcher, i < j
								&& matcherFromTokens(tokens.slice(i, j)), j < len && matcherFromTokens((tokens = tokens.slice(j))), j < len && tokens.join(""));
					}
	...
	jQuery.fn.extend({
		...
		before : function() {
			if (!isDisconnected(this[0])) { return this.domManip(arguments, false, function(elem) {
			...
			if (this[0]) {
				...
				if (first) {
					...
					for (iNoClone = results.cacheable || l - 1; i < l; i++) {
						callback.call(table && jQuery.nodeName(this[i], "table") ? findOrAppend(this[i], "tbody") : this[i], i === iNoClone ? fragment : jQuery.clone(fragment,
								true, true));
					}
				}
	...
	jQuery.buildFragment = function(args, context, scripts) {
		...
		if (args.length === 1 && typeof first === "string" && first.length < 512 && context === document && first.charAt(0) === "<" && !rnocache.test(first)
				&& (jQuery.support.checkClone || !rchecked.test(first)) && (jQuery.support.html5Clone || !rnoshimcache.test(first))) {
	...
	rdisplayswap = /^(none|table(?!-c[ea]).+)/, rmargin = /^margin/, rnumsplit = new RegExp("^(" + core_pnum + ")(.*)$", "i"), rnumnonpx = new RegExp("^(" + core_pnum
			+ ")(?!px)[a-z%]+$", "i"), rrelNum = new RegExp("^([-+])=(" + core_pnum + ")", "i"), elemdisplay = {
		BODY : "block"
	},
	...
	jQuery.each([
		...
		jQuery.cssHooks[name] = {
			...
			set : function(elem, value, extra) {
				return setPositiveNumber(elem, value, extra ? augmentWidthOrHeight(elem, name, extra, jQuery.support.boxSizing && jQuery.css(elem, "boxSizing") === "border-box")
						: 0);
			}
		};
	});
	...
	/*
	 * Prefilters 1) They are useful to introduce custom dataTypes (see ajax/jsonp.js for an example) 2) These are called: - BEFORE asking for a transport - AFTER param
	 * serialization (s.data is a string if s.processData is true) 3) key is the dataType 4) the catchall symbol "*" can be used 5) execution will start with transport dataType and
	...
	/*
	 * Handles responses to an ajax request: - sets all responseXXX fields accordingly - finds the right dataType (mediates between content-type and expected dataType) - returns
			...
			.ajaxPrefilter("json jsonp",
					function(s, originalSettings, jqXHR) {
						...
						var callbackName, overwritten, responseContainer, data = s.data, url = s.url, hasCallback = s.jsonp !== false, replaceInUrl = hasCallback
								&& rjsonp.test(url), replaceInData = hasCallback && !replaceInUrl && typeof data === "string"
	...
	/*
	 * Microsoft failed to properly implement the XMLHttpRequest in IE7 (can't request local files), so we use the ActiveXObject when it is available Additionally XMLHttpRequest
	 * can be disabled in IE7/IE8 so we need a fallback.
	 */
	function() {
