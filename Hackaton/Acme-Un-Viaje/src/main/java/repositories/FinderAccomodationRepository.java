...
public interface FinderAccomodationRepository extends JpaRepository<FinderAccomodation, Integer> {
	...
	@Query("select a from Accomodation a where (a.description like %?1% or a.address like %?1%) and (a.pricePerNight <= ?2) and a.maxPeople >= ?3 and (a.address like %?4% or a.place like %?4%)")
