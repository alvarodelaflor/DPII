...
<!-- NO ESTÁ ACABADO, CARMEN -->
	...
	<display:table name="applications" id="row" requestURI="${requestURI}" pagesize="5" class="displaytag">
		...
		<display:column titleKey="application.curricula.show">
			<a href="curricula/show.do?curriculaId=${row.curricula.id}"><jstl:out value="${row.curricula.name}"/></a>
		</display:column>
		
		<display:column titleKey="application.problem">
			<a href="problem/rookie/show.do?problemId=${row.problem.id}"><jstl:out value="${row.problem.title}"/></a>
		</display:column>
