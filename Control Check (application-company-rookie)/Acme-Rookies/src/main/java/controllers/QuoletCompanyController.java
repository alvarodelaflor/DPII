...
public class QuoletCompanyController extends AbstractController {
	...
	public ModelAndView list(@RequestParam(defaultValue = "-1", value = "applicationId") final int applicationId) {
		...
		try {
			...
			res.addObject("requestURI", "quolet/company/list.do?applicationId=" + applicationId);
			res.addObject("applicationId", applicationId);
			// ALVARO
			final Application application = this.applicationService.findOne(applicationId);
			final Company companyApplication = application.getPosition().getCompany();
			final Company companyLogger = this.companyService.getCompanyByUserAccountId(LoginService.getPrincipal().getId());
			res.addObject("validToCreate", companyApplication.equals(companyLogger));
			// ALVARO
	...
	public ModelAndView createPOST(final Quolet quolet, final BindingResult binding) {
		...
		if (binding.hasErrors()) {
			...
			try {
				...
				res = new ModelAndView("redirect:/quolet/company/list.do?applicationId=" + quolet.getApplication().getId());
			} catch (final Throwable oops) {
				System.out.println("Error: " + oops);
				res = new ModelAndView("redirect:/welcome/index.do");
			}
