...
public interface QuoletRepository extends JpaRepository<Quolet, Integer> {
	...
	@Query("select x from Quolet x where x.application.id = ?1 and x.application.rookie.id = ?2")
	...
	@Query("select x from Quolet x where x.id = ?1 and x.application.rookie.id = ?2")
	Quolet getLoggedQuolet(int quoletId, int rookieId);
