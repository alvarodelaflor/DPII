...
<section id="main-content">
	<article>
		<div class="content">
			<form:form class="formularioEdicion" method="POST" modelAttribute="quolet" action="${URI}?applicationId=${applicationId}">
				<form:hidden path="id" />
					...
					<tr>
						...
						<jstl:if test="${quolet.id != 0}">
							<td><br> <input type="button" value="<spring:message code='problem.delete' />" name="problem.delete"
								onclick="window.location = 'quolet/rookie/delete.do?quoletId=${quolet.id}'" /></td>
						</jstl:if>
					</tr>
				</table>

				<acme:submit name="save" code="quolet.save" />
				<input type="button" value="<spring:message code='problem.back' />" onclick="window.location = 'quolet/rookie/list.do?applicationId=${quolet.application.id}'" />
			</form:form>
		</div>
	</article>
</section>
