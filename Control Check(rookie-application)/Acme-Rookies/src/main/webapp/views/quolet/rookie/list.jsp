...
<jstl:if test="${validToCreate}">
	<input type="button" value="<spring:message code='quolet.create' />" onclick="window.location = 'quolet/rookie/create.do?applicationId=${applicationId}'" formmethod="get"/>
</jstl:if>
<!-- ALVARO -->
	...
	<!-- ALVARO -->
		<display:column titleKey="quolet.edit">
			<jstl:if test="${row.draftMode}">
				<a href="quolet/rookie/edit.do?quoletId=${row.id}&applicationId=${applicationId}"><spring:message code="quolet.edit" /></a>
			</jstl:if>
		</display:column>
		<display:column titleKey="quolet.delete">
			<jstl:if test="${row.draftMode}">
				<a href="quolet/rookie/delete.do?quoletId=${row.id}"><spring:message code="quolet.delete" /></a>
			</jstl:if>
		</display:column>
	<!-- ALVARO -->
	</jstl:if>
