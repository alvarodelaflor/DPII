...
<body>
   ...
   <display:table name="msgs" id="row"  requestURI="${requestURI}"	pagesize="5" class="displaytag" >
  	...
  	<display:column property="moment" titleKey="msg.moment"/>
  	 ...
  	 <display:table name="messageBoxes" id="rowM"  requestURI="${requestURI}"	pagesize="5" class="displaytag" >
  	
  	<display:column titleKey="administrator.showMessageBox"> 
          			<a href="message/list.do?messageBoxId=${rowM.id}"><spring:message code="seeMessages" /></a>
	</display:column>
  	<c:choose>
   		<c:when test="${rowM.isDefault != true}">
    <display:column titleKey="administrator.editMessageBox"> 
		<a href="messageBox/edit.do?messageBoxId=${rowM.id}"><spring:message code="editMessageBox" /></a>
	</display:column>
    	</c:when>    
    	<c:otherwise>
        	<display:column titleKey="administrator.editMessageBox"/>
    ...
    <display:column titleKey="administrator.deleteMessageBox"> 
		<a href="messageBox/delete.do?messageBoxId=${rowM.id}"><spring:message code="deleteMessageBox" /></a>
	</display:column>
    	</c:when>    
    	<c:otherwise>
        	<display:column titleKey="administrator.deleteMessageBox"/>
  	...
  	<p class="create"><input type="button" value=<spring:message code="createMessageBox" /> id="buttonMessageBox" name="buttonMessageBox"  onclick="location.href='messageBox/createFolder.do?messageBoxId=${messageBoxId}';"/></p>
