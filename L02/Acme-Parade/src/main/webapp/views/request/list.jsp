...
<display:table name="requests" id="row" requestURI="${requestURI}" pagesize="5" class="displaytag">
	...
	<display:column property="positionAux.parade.title" titleKey="parade.title"/>
	<display:column	titleKey="request.show">
		<a href="/Acme-Parade/request/member/show.do?requestId=${row.id}"><spring:message code="request.show"/></a>
	</display:column>
	<display:column	titleKey="request.delete">
		<jstl:if test="${row.status eq pending}">
				<a href="/Acme-Parade/request/member/delete.do?requestId=${row.id}"><spring:message code="request.delete"/></a>
		</jstl:if>
	</display:column>
</display:table>
<form method="get">
